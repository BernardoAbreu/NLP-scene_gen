...
for FILE in $INPUT_DIR/*; do
	BASEFILE=$(basename $FILE);
	echo $BASEFILE;
	sed -E "s/^[ \t]+([A-Z][A-Z\'\"\-\.0-9 ]*) *(\([a-zA-Z0-9\'\"\. ]+\))?$/<CHAR>__{\L\1}/g" "${FILE}" > "${OUTPUT_FILE}/${BASEFILE}";
...
for FILE in $INPUT_DIR/*; do
	BASEFILE=$(basename $FILE);
	echo $BASEFILE;
	sed -i -E ":begin;$!N;s/^(<LOC>__\{[^\n]*) *\}\n<LOC>__\{([^\n]*\})/\1 \2/;tbegin;P;D" "${OUTPUT_FILE}/${BASEFILE}";
...
for FILE in $INPUT_DIR/*; do
	BASEFILE=$(basename $FILE);
	echo $BASEFILE;
	sed -i -E "s/^<LOC>__/<L>__/g" "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E "s/^<TRANS>__/<T>__/g" "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E "s/^<ACTION>__/<A>__/g" "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E "s/^<ACTION_ALT>__/<B>__/g" "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E "s/^<DIAL>__/<D>__/g" "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E "s/^<DIAL_PAR>__/<P>__/g" "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E "s/^<CHAR>__/<C>__/g" "${OUTPUT_FILE}/${BASEFILE}";
	# sed -i -E "s/^<CHAR_PAR>__/<E>__/g" "${OUTPUT_FILE}/${BASEFILE}";
	# sed -i -E '/<E>__\{\}$/d' "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E 's/\^\~/ /g' "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E 's/[\-\-]/ /g' "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E 's/[\\\-\/]/ /g' "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E 's/\s+/ /g' "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E "s/i'll/i will/g" "${OUTPUT_FILE}/${BASEFILE}";
	sed -i -E "s/(int|ext)\./\1erior/g" "${OUTPUT_FILE}/${BASEFILE}";
done;
